...
//크기가 1x1인 정사각형의 네 꼭짓점이 모두 드래곤 커브의 일부인 정사각형 개수 구하기
int cntSquare(vector<vector<bool>>& board) {
    int ans = 0;
    for (int i = 0; i < SIZE - 1; i++) {
        for (int j = 0; j < SIZE - 1; j++) {
            //드래곤 커브의 일부인 1x1 정사각형이 되는 경우
...
ci makeDragonCurve(vector<vector<bool>>& board, vector<int>& direc, ci point) {
    ...
    //한 세대의 방향정보를 역순으로 탐색하면서 +1%4 (0~3세대) 한 값이 그 다음 세대 방향정보
    for (int i = size - 1; i >= 0; i--) {
        int new_direc = (direc[i] + 1) % 4; //현재 시작 점의 방향
        point = { point.first + dy[new_direc], point.second + dx[new_direc] }; //현재 끝 점
        board[point.first][point.second] = true;//선분이 그어짐.
        direc.push_back(new_direc);//다음 세대의 방향정보를 구하기 위해 현재 방향 정보 저장
    }
    return point;
}
...
int main() {
    int n, x, y, d, g;//n: 드래곤 커브 개수, x,y: 드래곤 커브 시작점, d: 시작 방향, g:세대
