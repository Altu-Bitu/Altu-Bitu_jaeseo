...
vector<vector<ci>> findCard(vector<vector<int>>& board) { //존재하는 모든 카드의 위치를 리턴하는 함수
    vector<vector<ci>> card_pos(7, vector<ci>(0)); //최대 카드 수 //카드 위치 저장
    int cnt = 0;
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            cnt = max(cnt, board[i][j]); // 존재하는 카드 수
...
int bfs(int r1, int c1, int r2, int c2, vector<vector<int>>& tmp) { //현재 커서에서 목표 카드로 이동하는 가장 적은 비용을 리턴하는 함수
    vector<vector<int>> visited(SIZE, vector<int>(SIZE, 0)); //방문여부(키 조작 횟수로 표시)
    ...
    while (!q.empty()) {
        ...
        int dist = visited[row][col];//최소 키 조작 횟수
        q.pop();

        if (row == r2 && col == c2) //목표 카드에 도달했다면
            return dist;//최소 키 조작 횟수 리턴

        for (int i = 0; i < 4; i++) { //컨트롤 입력 이동
            pair<int, int> np = ctrl(row, col, i, tmp);//이동할 좌표
            if (visited[np.first][np.second])//방문여부
                continue;
            visited[np.first][np.second] = dist + 1; //거리, 방문 갱신
            q.push(np);
        }
...
int solution(vector<vector<int>> board, int r, int c) {
    ...
    for (int i = 0; i < card_cnt; i++)
        seq[i] = i + 1; // 1, 2, 3, ...(각 캐릭터 숫자)

    do { //가능한 모든 카드 순서에 대해
        for (int bit = 0; bit < (1 << card_cnt); bit++) { //0, 1번째 카드 중 어떤 카드를 먼저 뒤집을지 결정
            answer = min(answer, matchCard(bit, card_cnt, r, c, seq, cards, board));//현재 카드 순서와 각 카드를 뒤집는 순서에 대한 커서 이동 횟수 계산 후, 최소로 갱신
        }
    } while (next_permutation(seq.begin(), seq.end()));//존재하는 카드 다 돌 때까지
    return answer;
}
 ...
 */
int main() {
    vector<vector<int>> board = { {1, 0, 0, 3},// 카드 초기화(예시)
                                 {2, 0, 0, 0},
                                 {0, 0, 0, 2},
                                 {3, 0, 1, 0} };
